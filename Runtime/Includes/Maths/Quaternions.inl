...
namespace kbh
{
	...
	RadianAngle<T> Quat<T>::AngleBetween(const Quat& quat) const
	{
		...
		return std::acos(kbh::Clamp(alpha, T(-1.0), T(1.0)));
	}
	...
	{
		Quat interpolated;
		interpolated.w = kbh::Lerp(from.w, to.w, interpolation);
		interpolated.x = kbh::Lerp(from.x, to.x, interpolation);
		interpolated.y = kbh::Lerp(from.y, to.y, interpolation);
		interpolated.z = kbh::Lerp(from.z, to.z, interpolation);
